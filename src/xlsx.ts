...
export class Workbook {
  ...
  private readonly _: IWorkbook;
  constructor(workbook) {
    this._ = workbook;
    this.sheetNames = workbook.worksheets.map((s) => s.name);
  }
  sheet(name, options) {
    const sname =
      typeof name === "number"
...
function extract(sheet: IWorksheet, {range, headers} = {}) {
  let [[c0, r0], [c1, r1]] = parseRange(range, sheet);
  const headerRow = headers
    ? // @ts-ignore
      sheet._rows[r0++]
    : null;
  let names: Array<number> | Set<string> = new Set(["#"]);
  for (let n = c0; n <= c1; n++) {
