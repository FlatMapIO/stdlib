...
import {isArqueroTable} from "./arquero";
import {isArrowTable, loadArrow} from "./arrow";
...
// is optional.
export function isDatabaseClient(value: ValueType, mode?: "table") {
  return (
    value &&
...
export async function loadDataSource(source, mode, name) {
  switch (mode) {
    case "table":
      return loadTableDataSource(source, name);
    case "sql":
      return loadSqlDataSource(source, name);
  }
  return source;
}
...
const loadTableDataSource = sourceCache(
  async (source: FileAttachment, name) => {
